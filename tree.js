...
define(["require", "exports", "knockout", "jquery", "./utils", "./contextmenu", "./engine", "jqueryui"], function (require, exports, ko, $, utils, ctx, engine_1) {
    "use strict";
    var STATE_CACHE_KEY = "__SPA_TREE_STATE__", STATE_ACTIVE_KEY = "__SPA_TREE_ACTIVE_ITEM__";
    exports.defaults = {
        cssClass: "ui-tree",
        isDraggable: true,
        isDropTarget: true,
    ...
    var Tree = (function () {
        ...
        Tree.prototype.selectNode = function (id, root) {
            var level = root || this, children = level.children(), i = 0, len = children.length, node;
            for (; i < len; i++) {
                node = children[i];
                if (node.id() === id) {
                    node.selectNode();
    ...
    var TreeNode = (function () {
        function TreeNode(options, parent, viewModel, index) {
            ...
            this.isDraggable = ko.pureComputed(function () {
                var childRenaming = _this.children().some(function (child) { return child.isRenaming(); }), typeDefault = typeValueOrDefault("isDraggable", _this.type(), _this.viewModel);
        ...
        TreeNode.prototype.loadState = function () {
            ...
            var state = JSON.parse(localStorage.getItem(STATE_CACHE_KEY)), uid = this.uniqueIdentifier();
            if (state) {
    ...
    ko.bindingHandlers.tree = {
        ...
        update: function (element, valueAccessor) {
            var value = ko.unwrap(valueAccessor());
            ko.renderTemplate("text!koui/tree/container.html", value, { templateEngine: engine_1.defaultInstance }, element);
